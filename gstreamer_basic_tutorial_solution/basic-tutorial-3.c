...
int tutorial_main(int argc, char *argv[]) {
  ...
  data.video_convert =
      gst_element_factory_make("autovideosink", "video_convert");

  /* Create the empty pipeline */
  data.pipeline = gst_pipeline_new("test-pipeline");

  if (!data.pipeline || !data.source || !data.convert || !data.resample ||
      !data.sink || !data.video_convert) {
  ...
  gst_bin_add_many(GST_BIN(data.pipeline), data.source, data.convert,
                   data.resample, data.sink, data.video_convert, NULL);
...
//       gst_element_get_static_pad(data->video_convert, "video_convert");
...
//   g_print("Receive pad_type %s", new_pad_type);
//   g_print("Has type '%s'.\n", new_pad_type);
